...
def infix_to_postfix(task_string):
    ...
    if task_string[0] == "-":
        task_string = "0" + task_string
    elif task_string[0] == "+":
        task_string = task_string[1:]
    elif task_string[0] == "*":
        return -1
    task_string = re.sub(r"\(\s*?-", "(0-", task_string)
    task_string = re.sub(r"\(\s*?\+", "(", task_string)
    # Поиск ошибочных ситуаций
    bad_symbols = re.findall(r"[^\d*\s\-\+)(]", task_string)
    bad_symbols += re.findall(r"\d\s+\d", task_string)
