...
def sort_pyramidal(n, sequence):
    '''
    :param n: длина последовательности
    :param sequence: последовательность чисел
    :return sort_sequence: отсортированная последовательность чисел
    '''
    heap = Heap()
    sort_sequence = []
    ...
    while heap.size() > 0:
        item = heap.extract()
        sort_sequence.append(item)
    sort_sequence.reverse()
...
def main():
    ...
    # Пирамидальная сортировка
    sort_sequence = sort_pyramidal(n, sequence)
    # Записываем результат в output.txt
    save_output(OUTPUT_FILE, " ".join(list(map(str, sort_sequence))))
