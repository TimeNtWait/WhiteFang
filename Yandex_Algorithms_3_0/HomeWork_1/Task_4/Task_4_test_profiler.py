...
def calc_exam(n_students, n_variants, n_row, side):
    ...
    if n_variants >= n_students:
        return [-1]
    # не требуется так как по условию n_variants >= 2
    # # Если вариант всего 1, тогда можно сесть за туже парту
    # if n_variants == 1:
    #     return [n_row, (2 - (side-1))]
...
def test_calc_exam(numbers, target_result_exam):
    result_exam = calc_exam(*numbers)
    assert result_exam == target_result_exam
