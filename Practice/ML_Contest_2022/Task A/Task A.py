...
def load_input_data(input_filename):
    with open(input_filename, 'r') as file:
        ...
        is_covid_by_employees = [int(n) for n in rows[1].split(" ")]
    ...
    return count_employees, is_covid_by_employees, meetings_by_employees

def main():
	# Загружаем входные данные
	count_employees, is_covid_by_employees, meetings_by_employees = load_input_data(input_filename)

	# Формируем словарь для учета встреч
	meetings_dict = {}
	for empl_index in range(count_employees):
	    ...
	    for meet in meetings:
	        if meet in meetings_dict:
	            ...
	            meetings_dict[meet] = [empl_index]	
	# Определяем новый список зараженных (по умлочанию туда входят те кто уже заражен)
	new_is_covid_by_employees = is_covid_by_employees.copy()
	...
	for num_meet in sorted(list(set(meetings_dict))):
	    ...
	    for empl_index in meetings_dict[num_meet]:
	        if (new_is_covid_by_employees[empl_index] == 1):
	            meeting_is_covid = True
	            break
	    # если встреча ковидная, то все её участники заражены
	    if meeting_is_covid:
	        for empl_index in meetings_dict[num_meet]:
	            new_is_covid_by_employees[empl_index] = 1

	string_new_is_covid_by_employees = " ".join(str(n) for n in new_is_covid_by_employees)
	# Записываем результат в output.txt
	with open(output_filename, "w") as file:
	    file.write(string_new_is_covid_by_employees)
